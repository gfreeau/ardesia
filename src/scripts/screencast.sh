...
then
  ...
  if [ "$ICECAST" = "TRUE" ]
  then
    RECORDER_PROGRAM_OPTIONS="-vvv screen:// --ignore-config --screen-fps=12 :input-slave=alsa:// --sout  "#transcode{venc=theora,vcodec=theo,scale=0.7,acodec=vorb,channels=2,samplerate=44100,deinterlace,audio-sync}:duplicate{dst=std{access=shout,mux=ogg,dst=source:$ICECAST_PASSWORD@$ICECAST_ADDRESS:$ICECAST_PORT/$ICECAST_MOUNTPOINT},dst=std{access=file,mux=ogg,dst=$2}}"" 
  else
    RECORDER_PROGRAM_OPTIONS="-vvv screen:// --screen-fps=12 :input-slave=alsa:// --sout-theora-quality=5 --sout-vorbis-quality=1 --sout-transcode-deinterlace --sout "#transcode{venc=theora,vcodec=theo,scale=0.7,acodec=vorb,channels=2,samplerate=44100,deinterlace,audio-sync}:standard{access=file,mux=ogg,dst=$2}"" 
  fi
