...
then
  ...
  then
    ...
    RECORDER_PROGRAM_OPTIONS="-vvv screen:// --screen-fps=12 :input-slave=alsa:// --sout-theora-quality=5 --sout-vorbis-quality=1 --sout-transcode-deinterlace --sout "#transcode{venc=theora,vcodec=theo,vb=512,scale=0.7,acodec=vorb,ab=128,channels=2,samplerate=44100,deinterlace,audio-sync}:standard{access=file,mux=ogg,dst=$2}"" 
  fi
